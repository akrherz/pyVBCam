...
def camRunner(cid):
    ...
    # Our image may not be 640x480 at full res
    (_, imgheight) = i0.size
    draw.rectangle([5, imgheight - 5 - h, 5+w, imgheight - 5], fill="#000000")
    draw.text((5, imgheight - 5 - h), text, font=font)
    ...
    cmd = ("/home/ldm/bin/pqinsert "
           "-p 'webcam c %s camera/stills/%s.jpg camera/%s/%s_%s.jpg jpg'"
           " ../tmp/%s-320x240.jpg") % (gmt.strftime("%Y%m%d%H%M"), cid, cid,
                                        cid, gmt.strftime("%Y%m%d%H%M"), cid)
    proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
    proc.stderr.read()
    cmd = ("/home/ldm/bin/pqinsert "
           "-p 'webcam ac %s camera/640x480/%s.jpg camera/%s/%s_%s.jpg jpg'"
           " ../tmp/%s-640x480.jpg") % (gmt.strftime("%Y%m%d%H%M"), cid, cid,
                                        cid, gmt.strftime("%Y%m%d%H%M"), cid)
    proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
    proc.stderr.read()
